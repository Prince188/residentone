...
export default function PayMaintenancePage() {
  ...
  const createOrderMutation = useMutation({
    mutationFn: async () => {
      const res = await createRazorpayOrder(cycleId, unitId, 1);
    ...
    onSuccess: async (order) => {
      ...
      if (order.isMock) {
        ...
        try {
          ...
          setSuccess(
            `Payment successful (mock) — ₹${order.total} (₹${order.baseAmount} + ₹${order.fee} fee). Receipt generated.`
          );
  ...
  if (detailQuery.isError)
    return (
      <div className="p-6 text-center text-error">
        {extractApiError(detailQuery.error, "Load failed")}
      </div>
    );
  ...
  const roleLabel =
    r.isOwner ? "Owner" : r.isTenant ? "Renter" : r.houseRole === "owner" ? "Owner" : "Renter";
  ...
  return (
    <div className="mx-auto max-w-xl space-y-5 sm:space-y-6">
      ...
      <p className="page-subtitle">
        House {r.label} · {roleLabel} · {periodLabel(r.cycle.month, r.cycle.year, r.cycle.durationMonths)} · {formatAmount(base)} due
      </p>
        ...
        <div className="flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 p-4 text-body-sm font-semibold text-red-800">
          ...
          <div>
            ...
            <p className="mt-0.5 font-normal text-red-700">
              An overdue charge of {formatAmount(r.cycle.lateCharge)} has been added as the payment
              deadline ({formatDate(r.cycle.dueDate)}) has passed.
            </p>
          </div>
        </div>
      )}
      ...
      {isPaid ? (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-6 text-center">
          ...
          <p className="mt-2 text-body-md font-semibold text-emerald-800">
            Already paid — {formatAmount(r.totalAmount || base)} via {r.method}
          </p>
        ...
        <section className="rounded-xl border border-outline-variant bg-surface-container-lowest p-5">
          ...
          <div className="mt-4 space-y-3">
            <div className="rounded-xl border-2 border-primary bg-primary/5 p-4">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-auto rounded-full bg-primary px-2 py-0.5 text-label-sm text-on-primary">
                  Recommended
                </span>
              </div>
              <div className="mt-3 space-y-1 rounded-lg bg-white p-3 text-body-sm">
                <div className="flex justify-between">
                  <span className="text-on-surface-variant">Maintenance ({roleLabel})</span>
              ...
              <p className="mt-2 text-label-sm text-on-surface-variant">
                Society gets full {formatAmount(base)} ({roleLabel} rate) — Razorpay keeps fee. Instant
                receipt.
              </p>
              <button
                type="button"
                ...
                className="mt-3 w-full rounded-full bg-primary px-4 py-2.5 text-label-md font-semibold text-on-primary hover:opacity-90 disabled:opacity-50 cursor-pointer"
              >
              ...
              {feeInfo?.isMock && (
                <p className="mt-2 text-center text-label-sm text-amber-700">
                  Mock mode — no real Razorpay keys. Will simulate success for testing cash flow.
                </p>
              )}
            </div>
            ...
            <div className="rounded-xl border border-outline-variant bg-surface-container-low p-4">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-label-sm text-emerald-800">
                  No fee
                </span>
              </div>
              <p className="mt-2 text-body-sm text-on-surface-variant">
                Pay{" "}
                <span className="font-semibold text-on-surface">{formatAmount(base)} only</span>{" "}
                cash at society office. No extra fee. Admin will mark as{" "}
                <span className="font-semibold">Cash</span> and give receipt. Takes 1-2 hours.
              </p>
          ...
          {error && (
            <p className="mt-4 rounded-lg bg-error-container px-3 py-2 text-label-md text-on-error-container">
              {error}
            </p>
          )}
          {success && (
            <p className="mt-4 rounded-lg bg-emerald-50 px-3 py-2 text-label-md text-emerald-800">
              {success}
            </p>
          )}
